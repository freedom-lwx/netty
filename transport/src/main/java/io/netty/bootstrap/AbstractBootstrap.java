...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
	...
	/*
	 * 不管实在Bootstrap 还是 ServerBootstrap 中，其实都必有一个所谓的“主”EventLoopGroup和其对应的“主”channel
	 * 一下的属性都是与这些“主”元素想关联的属性
	 */
	...
	//这个Handler应该是一个主handler吧？默然会插入当前“主”channel的pipeline中
	...
	/**
	 * The {@link Class} which is used to create {@link Channel} instances from. You
	 * either use this or {@link #channelFactory(io.netty.channel.ChannelFactory)}
	 * if your {@link Channel} implementation has no no-args constructor.
	 */
	...
	/**
	 * @deprecated Use {@link #channelFactory(io.netty.channel.ChannelFactory)}
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * {@link io.netty.channel.ChannelFactory} which is used to create
	 * {@link Channel} instances from when calling {@link #bind()}. This method is
	 * usually only used if {@link #channel(Class)} is not working for you because
	 * of some more complex needs. If your {@link Channel} implementation has a
	 * no-args constructor, its highly recommend to just use {@link #channel(Class)}
	 ...
	 * If the {@code value} is {@code null}, the attribute of the specified
	 * {@code key} is removed.
	 */
	...
	/**
	 * Validate all the parameters. Sub-classes may override this, but should call
	 * the super method in that case.
	 */
	public B validate() {
		if (group == null) {
	 ...
	 * This method is useful when making multiple {@link Channel}s with similar
	 * settings. Please note that this method does not clone the
	 * {@link EventLoopGroup} deeply but shallowly, making the group a shared
	 * resource.
	 */
	@Override
	...
	private ChannelFuture doBind(final SocketAddress localAddress) {
		...
		if (regFuture.isDone()) {
			...
			// Registration future is almost always fulfilled already, but just in case it's
			// not.
			...
			regFuture.addListener(new ChannelFutureListener() {
				...
				public void operationComplete(ChannelFuture future) throws Exception {
					...
					if (cause != null) {
						// Registration on the EventLoop failed so fail the ChannelPromise directly to
						// not cause an
	...
	/**
	 * Returns the {@link AbstractBootstrapConfig} object that can be used to obtain
	 * the current config of the bootstrap.
	 */
	...
	private static void setChannelOption(Channel channel, ChannelOption<?> option, Object value,
		...
		try {
			...
			logger.warn("Failed to set channel option '{}' with value '{}' for channel '{}'", option, value, channel,
					t);
		}
	}
	...
	public String toString() {
		StringBuilder buf = new StringBuilder().append(StringUtil.simpleClassName(this)).append('(').append(config())
				.append(')');
		return buf.toString();
	}
	...
	static final class PendingRegistrationPromise extends DefaultChannelPromise {
		...
		protected EventExecutor executor() {
			...
			// The registration failed so we can only use the GlobalEventExecutor as last
			// resort to notify.
